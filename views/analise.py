...
st.markdown(
        "<p style='text-align: justify; color:gray; font-size:25px'>Os níveis de estoques de petróleo bruto e produtos refinados em todo o mundo atuam como um amortecedor para as flutuações de preços. Grandes estoques geralmente indicam uma oferta abundante, pressionando os preços para baixo. Baixos níveis de estoques podem levar a preocupações com escassez e podem aumentar os preços, mesmo com uma oferta estável. A capacidade de armazenamento também é um fator importante; se os estoques se aproximam da capacidade máxima, isso pode criar uma pressão adicional de baixa nos preços. Inversamente, se os níveis de estoques caírem abaixo de um certo nível considerado confortável, pode gerar preocupações sobre uma oferta insuficiente, impulsionando os preços.</p>",  unsafe_allow_html=True
    )
